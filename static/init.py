...
if run("dialog --defaultno --yesno \"Am I a worker?\" 5 19", shell=True, stderr=PIPE).returncode != 0:
    while True:
        ...
        url = "https://volterra.olinfo.it/phantom/collector/contestant?mac=" + mac + "&row=" + row + "&col=" + col
        r = requests.get(url)
    ...
    while True:
        ...
        url = "https://volterra.olinfo.it/phantom/collector/worker?mac=" + mac + "&num=" + num
        r = requests.get(url)
        if r.status_code != 200:
            run(f"dialog --msgbox \"Error: {r.text}\" 6 40", shell=True)
        else:
...
ts = ''
r = requests.get("https://volterra.olinfo.it/phantom/collector/reboot_timestamp")
if r.status_code != 200:
